...
async function generateTiles(config) {
    const { tile_size, input, palette, tiles, components, biomes, generation } = config;

    console.log(`[INFO] Loading template image from: ${input}`);
    const templateImage = await Jimp.read(input);
    console.log(`[INFO] Template image loaded. Dimensions: ${templateImage.bitmap.width}x${templateImage.bitmap.height}`);
    ...
    for (const task of generation) {
        ...
        console.log(`[INFO] Output dimensions: ${maxW}x${maxH} tiles -> ${outputWidth}x${outputHeight} pixels.`);

        const outputImage = await new Jimp({width: outputWidth, height: outputHeight, color: 0x00000000});
        ...
        for (const placement of preset) {
            ...
            for (const component of placementComponents) {
                ...
                console.log(`[DEBUG] Processing component '${component}' at tile coord (${placement.x}, ${placement.y}), pixel coord (${targetX}, ${targetY}).`);
                
                if (placement.base) {
                    const tileDef = tiles[placement.base];
                    if (!tileDef) {
                        console.error(`[WARN] Base tile '${placement.base}' for placement at (${placement.x}, ${placement.y}) not found. Skipping base.`);
                    } else {
                        const tileSourceX = tileDef.x * tile_size;
                        const tileSourceY = tileDef.y * tile_size;

                        let baseTile = await new Jimp({width: tile_size, height: tile_size, color: 0x00000000});

                        baseTile.blit({src: templateImage,x: 0, y: 0, srcX: tileSourceX, srcY: tileSourceY, srcW: tile_size, srcH: tile_size});
                        ...
                        console.log(`[INFO] Placed base tile '${placement.base}' underneath component.`);
                    }
                }
                ...
                let componentImg = await new Jimp({width: componentPixelW, height: componentPixelH, color: 0x00000000});

                componentImg.blit({src: templateImage,x: 0, y: 0, srcX: componentSourceX, srcY: componentSourceY, srcW: componentPixelW, srcH: componentPixelH});

                recolorImage(componentImg, colorMap);

                outputImage.blit({src: componentImg, x: targetX, y: targetY});

                console.log(`[INFO] Placed component '${component}' at (${placement.x}, ${placement.y}) (${targetX}, ${targetY}).`);
            }
        }
...
async function main() {
    ...
    if (!configPath) {
        console.error('ERROR: Please provide the path to the configuration file as a command line argument.');
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('Please ensure the config file path and the input image path are correct.');
